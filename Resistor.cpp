	...
	if (ptr2stepBro != -1) {
		if (ptr2stepBro < resistorArray.size()) {
			...
			if (horizontal) {
				...
				else { draw_line_inverted(broPos.x, broPos.y + al_get_bitmap_width(resistorImage), rPos.x, rPos.y + al_get_bitmap_width(resistorImage), al_map_rgb(LINECOLOUR), LINEWIDTH); }
			}
		}
